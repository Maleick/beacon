...
test -f "$WORKTREE_REPO/.autoship/workspaces/issue-156/config/model-routing.json" || fail "create-worktree copies runtime routing into the workspace"
if (
  cd "$WORKTREE_REPO"
...
(
  ...
  mkdir -p "$CONFIG_DIR"
  printf '%s\n' '{"plugin":["other-plugin","./plugins","opencode-autoship@latest","file:///tmp/stale-autoship.ts"],"customSetting":true}' >"$CONFIG_DIR/opencode.json"
  ...
  jq -e '.plugin | index("./plugins")' "$CONFIG_DIR/opencode.json" >/dev/null || fail "package installer preserves generic local plugins registration"
  jq -e '.plugin | index("opencode-autoship@latest") | not' "$CONFIG_DIR/opencode.json" >/dev/null || fail "package installer removes legacy opencode-autoship@latest registration"
  jq -e '.plugin | index("file:///tmp/stale-autoship.ts") | not' "$CONFIG_DIR/opencode.json" >/dev/null || fail "package installer removes legacy file autoship registration"
  ...
  test -f "$CONFIG_DIR/commands/autoship.md" || fail "package installer exposes /autoship command to OpenCode"
  test -f "$CONFIG_DIR/commands/autoship-setup.md" || fail "package installer exposes /autoship-setup command to OpenCode"
  test -f "$CONFIG_DIR/skills/autoship-setup/SKILL.md" || fail "package installer exposes autoship-setup skill to OpenCode"
  test -f "$CONFIG_DIR/skills/autoship-orchestrate/SKILL.md" || fail "package installer exposes autoship-orchestrate skill to OpenCode"
  ...
  assert_eq "$package_version" "$(tr -d '\n' <"$CONFIG_DIR/plugins/autoship.version")" "package installer writes legacy plugin version marker"
